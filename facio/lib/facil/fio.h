...
FIO_FUNC inline void fio_reschedule_thread(void) {
  ...
  //https://blog.csdn.net/hbuxiaofei/article/details/46416605
  nanosleep(&tm, NULL);
}
...
FIO_FUNC inline int fio_trylock(fio_lock_i *lock) {
  ...
  //编码器优化时  生成的汇编指令可能是乱序，导致访问错乱  加上这句就是确保顺序执行 总之复杂
...
***************************************************************************** */
//#if !defined == #ifndef #if defined()==#ifdef
...
inline FIO_FUNC fio_str_info_s fio_str_info(const fio_str_s *s) {
  if (!s)
    return (fio_str_info_s){.len = 0};//如果传递过来的对象是空值，就直接返回0[结构体]
  return (s->small || !s->data)//如果有samll||数据为空
             ? (fio_str_info_s){.capa =
                                    (s->frozen ? 0 : (FIO_STR_SMALL_CAPA - 1)),/**冻结了直接返回0**/
                                .len = (size_t)(s->small >> 1),
                                .data = FIO_STR_SMALL_DATA(s)}
             : (fio_str_info_s){.capa = (s->frozen ? 0 : s->capa),/**冻结了返回0，否则返回总容量，返回长度，返回数据，如果有的话**/
                                .len = s->len,
                                .data = s->data};
}
 ...
 */
inline FIO_FUNC fio_str_info_s fio_str_resize(fio_str_s *s, size_t size) {
    //s为空||s被冻结 被冻结了也不会申请内存了
  if (!s || s->frozen) {
    return fio_str_info(s);
  }
  if (s->small || !s->data) {
    if (size < FIO_STR_SMALL_CAPA) {//FIO_STR_SMALL_CAPA 3744 行  最小容量

        //size 左移1位  | 1 && 1111 11111
        //typedef unsigned char   uint8_t; 1个字节  无符号
      s->small = (uint8_t)(((size << 1) | 1) & 0xFF);

      //FIO_STR_SMALL_DATA(s) 强制转换为字符串地址 s=字符串地址
      //3740 行  =((char *)((&(s)->frozen) + 1))
    ...
    //fio_str_capa_assert 返回 fio_str_info_s   体  4018行
...
inline FIO_FUNC fio_str_info_s fio_str_write(fio_str_s *s, const void *src,
  ...
  if (!s || !src_len || !src || s->frozen)
    return fio_str_info(s);//如果s参数是空值或是长度没有或是是冻结，，直接返回字符串对象信息【容量，，长度，数据】

  //fio_str_len(s) 得到字符串的长度+现给长度 fio_str_resizefio_str_resize 3928行
  //fio_str_resize 返回 fio_str_info_s 【容量，长度，data的更新】
