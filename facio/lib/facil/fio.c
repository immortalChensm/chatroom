...
static inline int fio_clear_fd(intptr_t fd, uint8_t is_open) {
  fio_packet_s *packet;//139 行定义的结构体
  fio_protocol_s *protocol;//545行定义
  fio_rw_hook_s *rw_hooks;//1364行定义
  void *rw_udata;
  fio_uuid_links_s links;
  //锁定 其它线程阻塞【内存屏障和原子操作实现】
...
//任务队列链表
//每个结点都是一个任务队列结点   [每个结点都有大量的任务数组对象]
//多个任务队列结点构成任务队列链表
/* task queue block */
typedef struct fio_defer_queue_block_s fio_defer_queue_block_s;
struct fio_defer_queue_block_s {
  fio_defer_task_s tasks[DEFER_QUEUE_BLOCK_COUNT];//当前结点的任务列表
  fio_defer_queue_block_s *next;//下一个结点
  size_t write;//读写
  size_t read;
...
void fio_expected_concurrency(int16_t *threads, int16_t *processes) {
  ...
  } else if (*threads < 0 || *processes < 0) {//线程数和进程数量未给
...
static void deferred_on_close(void *uuid_, void *pr_) {
  ...
  if (pr->rsv)
    goto postpone;
  pr->on_close((intptr_t)uuid_, pr);//关闭客户端
  return;
postpone:
...
static intptr_t fio_tcp_socket(const char *address, const char *port,
  ...
  //https://blog.csdn.net/qiaoliang328/article/details/4729869
  hints.ai_family = AF_UNSPEC;     // don't care IPv4 or IPv6 不会限制IPV4,IPV6 适用于主机名和服务名且适用于任何协议的地址
  hints.ai_socktype = SOCK_STREAM; // TCP stream sockets socket流类型
  hints.ai_flags = AI_PASSIVE;     // fill in my IP for me  socket地址用于监听和绑定

  //得到的addrinfo 结构体内容赋值给addrinfo结构体变量
  //将地址，端口映射到一个地址  《unix环境高级编程第三版》第492页
  ...
  if (server) { //是否作为服务器 此参数传递了表示作为服务器运行  否则作为客户端运行
    {
      // avoid the "address taken" 地址复用选项配置
      int optval = 1;
...
socket_okay:
  fio_lock(&fd_data(fd).protocol_lock);//当前文件描述符协议【连接结构体 line 153】绑定
  fio_clear_fd(fd, 1);
  fio_unlock(&fd_data(fd).protocol_lock);
...
static int fio_attach__internal(void *uuid_, void *protocol_) {
  ...
  //当前文件描述符号的协议对象 struct fio_protocol_s
...
static void *fio_sentinel_worker_thread(void *arg) {
  ...
  if (child == -1) {
    ...
    fio_stop();
    return NULL;
  } else if (child) {//为值是父进程
    int status;
    waitpid(child, &status, 0);//等待回收子进程  总之让子进程先运行
#if DEBUG
...
static void fio_listen_on_startup(void *pr_) {
  ...
  fio_listen_protocol_s *pr = pr_;
  //添加事件句柄【sockfd 】与事件监听  每个文件描述符绑定的协议
...
static void fio_listen_on_data(intptr_t uuid, fio_protocol_s *pr_) {
  ...
  for (int i = 0; i < 4; ++i) {
    intptr_t client = fio_accept(uuid);//接受客户端
    if (client == -1)
      return;
    pr->on_open(client, pr->udata);//运行用户的on_open指定的函数 pr->udata 用户自定义参数【http协议时启用】
  }
}
...
intptr_t fio_listen FIO_IGNORE_MACRO(struct fio_listen_args args) {
  ...
  //line 2575 创建完socket 后返回处理好的fd[uuid]
  //如果是作为服务则是已经监听的socket 如果是客户端则是已经连接服务器的socket了
