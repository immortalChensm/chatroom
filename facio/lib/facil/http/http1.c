...
static int http1_http2websocket_server(http_s *h, websocket_settings_s *args) {
  ...
  //websocket 握手协议时需要拼接这个固定的字符串+sha+base64返回给客户端
  ...
  if (stmp.len != 2 || stmp.data[0] != '1' || stmp.data[1] != '3')//检测websocket版本
    goto bad_request;

  tmp = fiobj_hash_get2(h->headers, sec_key);//得到客户端握手sec-key
  if (!tmp)
    goto bad_request;
