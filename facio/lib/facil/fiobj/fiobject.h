...
#define FIOBJECT_TYPE_MASK (~(uintptr_t)7)//0000 0111 取反1111 1000 F8 高位变1 低位变0
#endif
...
/**
 * 获取FIOBJ o的类型  FIOBJ 它是uintptr_t的别称 即long long int 指针【地址】仅仅是个整数
 * @param o o 参数是个地址【整数】
 * @return
 */
FIO_INLINE fiobj_type_enum fiobj_type(FIOBJ o) {
  if (!o)
    return FIOBJ_T_NULL;//空指针
  if (o & FIOBJECT_NUMBER_FLAG)//相与操作  规则：当完全一样的时候才会为1
    return FIOBJ_T_NUMBER;//数字指针  即地址上的内容是数字
  if ((o & FIOBJECT_PRIMITIVE_FLAG) == FIOBJECT_PRIMITIVE_FLAG)
    return (fiobj_type_enum)o;//原始指针
  if (FIOBJECT_STRING_FLAG &&(o & FIOBJECT_PRIMITIVE_FLAG) == FIOBJECT_STRING_FLAG)
    return FIOBJ_T_STRING;//0110 0010 如果这个o是0010即使与0110相与运算结果也是没有变依然=0010 ，此时再和0010进行与逻辑得到1 返回字符串类型的地址
...
//这里定义的结构体变量  头文件【具体看http.h 引入fiobj.h【它在引入大量的数据类型文件】注意看引入顺序
...
/**
 * 根据参数 o它是个地址【指针】 判断类型后返回fiobj_object_vtable_s 地址
 * @param o
 * @return
 */
FIO_INLINE const fiobj_object_vtable_s *fiobj_type_vtable(FIOBJ o) {
  switch (FIOBJ_TYPE(o)) {//获取对象的类型 257行
  case FIOBJ_T_NUMBER:
...
FIO_INLINE fio_str_info_s fiobj_obj2cstr(const FIOBJ o) {
  ...
  //具体看http.h所引入的头文件顺序 可知
  //返回fiobj_object_vtable_s 变量的地址【首先经过类型确认】
  //to_str 调用fiobj_str.c 98行的成员函数
