...
//对象o #define FIOBJ2PTR(o) ((void *)((o)&FIOBJECT_TYPE_MASK))
//首先与掩码1111 1000 F8[假设cpu为2个字节】相与 返回后再强制转换为 fiobj_str_s
//后面的3位为0  3位二进制可表示7种数据
//在字符串创建成功时，返回的地址和2相与  现在在和低字节的000位相与去掉 从而得到正确的数据地址【指针】
//对象地址转换为字符串地址
...
***************************************************************************** */
/**
 * 根据o参数的地址，低字节相与000去掉得到正确的数据地址并返回字符串的信息fio_str_info_s
 * @param o
 * @return
 */
...
FIOBJ fiobj_str_new(const char *str, size_t len) {
  ...
  *s = (fiobj_str_s){
      ...
      .str = FIO_STR_INIT,//初始化 该宏位于fio.h 3430 初始化该结构体的成员
  };
  //给了字符串和长度时写入保存
  if (str && len) {
    //数据保存【多次写就追加到字符串后】
...
size_t fiobj_str_write(FIOBJ dest, const char *data, size_t len) {
  assert(FIOBJ_TYPE_IS(dest, FIOBJ_T_STRING));//检测是否是字符串
  if (obj2str(dest)->str.frozen)//是否冻结
    return 0;
  obj2str(dest)->hash = 0;
